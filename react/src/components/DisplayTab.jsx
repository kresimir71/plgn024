...
const DisplayTab = ({ formData, setFormDataWrapper, error }) => {
    ...
    return (
        <div className="wrap">
            ...
            <h2 className="nav-tab-wrapper">
                ...
                <a
                    href="#"
                    className={`nav-tab ${activeTab === 'example1module' ? 'nav-tab-active' : ''}`}
                    onClick={() => handleTabClick('example1module')}
                >
                    {__('Example 1 module Settings', plugin().textDomain)}
                </a>
                <a
                    href="#"
                    className={`nav-tab ${activeTab === 'example2module' ? 'nav-tab-active' : ''}`}
                    onClick={() => handleTabClick('example2module')}
                >
			...
			/>			
                ...
                {activeTab === 'example1module' && (
                    <div>
                        <h3>{__('Example 1 Module Settings', plugin().textDomain)}</h3>
			<DisplayExample1ModuleFormFields 
			    formData={formData} 
			    setFormDataWrapper={setFormDataWrapper} 
			    error={error}
			/>			
                ...
                {activeTab === 'example2module' && (
                    <div>
                        <h3>{__('Example 2 Module Settings', plugin().textDomain)}</h3>
			<DisplayExample2ModuleFormFields 
			    formData={formData} 
			    setFormDataWrapper={setFormDataWrapper} 
			    error={error}
			/>			
