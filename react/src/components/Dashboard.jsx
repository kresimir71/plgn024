...
const Dashboard = () => {
    ...
    const setFormDataWrapper = (newFormData) => { setSaveSuccess(""); setFormData(newFormData);};
    
    const handleSubmit = (e) => {
        e.preventDefault();
        const { errors, cumulativeErrors } = validateOnSubmit(formData);
        ...
        (err) => {
            setError({ _cumulative_error: __('Failed to save settings.', yourTextDomain) });
        }).catch(() => {
            setError({ _cumulative_error: __('Failed to save settings.', yourTextDomain) });
        }).finally(() => {
            setIsPending(false);
    ...
    return (
		...
		<>{error._cumulative_error && <div className="notice notice-error notice-alt" dangerouslySetInnerHTML={{ __html: error._cumulative_error }} />}</>
